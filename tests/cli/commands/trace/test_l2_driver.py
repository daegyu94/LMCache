...
def test_plan_supports_group_layout_descs(tmp_path):
    trace = tmp_path / "l2.lct"
    keys = [
        _key(13),
        ObjectKey(
            chunk_hash=(14).to_bytes(4, "big"),
            model_name="test",
            kv_rank=0,
            object_group_id=1,
        ),
            ...
            (
                EventType.L2_LOOKUP_TASK_SUBMITTED,
                {
                    **common,
                    ...
                    "task_id": 1,
                    "keys": keys,
                    "group_layout_descs": {0: _layout(4096), 1: _layout(8192)},
                },
            ),
            (
                EventType.L2_LOOKUP_TASK_COMPLETED,
    ...
    assert plan.prepare_objects == {keys[0]: 4096, keys[1]: 8192}

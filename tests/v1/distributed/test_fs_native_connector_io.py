...
def test_odirect_handles_unaligned_buffer_and_length(tmp_path):
    ...
    try:
        ...
        key = "test-model@00000000@0@44556677"

        write_storage = bytearray(len(payload) + 1)
        ...
        read_storage = bytearray(len(payload) + 1)
        ...
        data_path = tmp_path / "test-model@0x00000000@0@44556677.data"
        assert data_path.stat().st_size % os.statvfs(tmp_path).f_bsize == 0
    finally:
        client.close()

...
def test_raw_block_fdp_disabled_ignores_placement_handles() -> None:
    config = RawBlockL2AdapterConfig(
    ...
    assert config.fdp_placement_handles is None


def test_raw_block_fdp_from_dict_validates_enabled_handle_elements() -> None:
    with pytest.raises(ValueError, match="fdp_placement_handles must contain"):
...
def test_raw_block_fdp_status_reports_registered_nonzero_handles() -> None:
    ...
    try:
        ...
        assert status["fdp_placement_handles"] == [1, 7]
    finally:
        adapter.close()
...
def test_raw_block_fdp_status_preserves_configured_handle_order() -> None:
    fake_core = _FakeFdpCore(status=[(0, 10), (1, 11), (7, 17)])
    adapter = _make_fdp_adapter(
        fake_core,
        _make_fdp_config(placement_handles=[7, 1]),
    )
    try:
        status = adapter.report_status()
        assert status["fdp_placement_handles"] == [7, 1]
    finally:
        adapter.close()


def test_raw_block_fdp_rejects_user_placement_handle_zero() -> None:
    with patch(
        ...
        with pytest.raises(ValueError, match="must not contain 0"):
            _make_fdp_config(placement_handles=[0, 1])

    warning.assert_called_once()
    assert "placement handle 0" in warning.call_args.args[0]


def test_raw_block_fdp_user_handles_must_match_device_handles() -> None:
    fake_core = _FakeFdpCore(status=[(0, 10), (1, 11), (7, 17)])

    with pytest.raises(RuntimeError, match="does not match device"):
        _make_fdp_adapter(fake_core, _make_fdp_config(placement_handles=[1]))


def test_raw_block_fdp_requires_nonzero_device_handles() -> None:
    fake_core = _FakeFdpCore(status=[(0, 10)])

    with pytest.raises(RuntimeError, match="no non-zero handles"):

...
def test_raw_block_fdp_from_dict_validates_enabled_placement_id_elements() -> None:
...
def test_raw_block_fdp_empty_status_fails_startup():
    ...
    with pytest.raises(RuntimeError, match="no placement identifiers"):
        _make_fdp_adapter(fake_core, config)


def test_raw_block_fdp_status_reports_registered_nonzero_placement_ids() -> None:
...
def test_raw_block_fdp_status_preserves_configured_placement_id_order() -> None:
    fake_core = _FakeFdpCore(status=[(0, 10), (1, 11), (7, 17)])
    adapter = _make_fdp_adapter(
        fake_core,
...
def test_raw_block_fdp_user_ids_must_match_device_ids() -> None:
    fake_core = _FakeFdpCore(status=[(0, 10), (1, 11), (7, 17)])
...
def test_raw_block_fdp_requires_nonzero_device_placement_ids() -> None:
    fake_core = _FakeFdpCore(status=[(0, 10)])

    with pytest.raises(RuntimeError, match="no non-zero placement identifiers"):

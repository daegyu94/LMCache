...
The FDP probe uses its own variable (separate from the ``io_uring_cmd`` tests'
``LMCACHE_TEST_CHAR_DEVICE``) so a non-FDP NVMe device configured for the
uring_cmd tests does not accidentally trigger FDP status queries. When
``LMCACHE_TEST_FDP_CHAR_DEVICE`` is not set, the test skips. If the configured
device, kernel, or controller does not support the FDP status query, the test
skips with the underlying capability error. A passing status probe only
confirms that the live device can answer the FDP status query; full adapter
initialization on hardware and KV write placement are separate follow-ups.
